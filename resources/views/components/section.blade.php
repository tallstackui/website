...
>
    @if ($title)
        <div @class(['flex items-center justify-between', 'mb-2' => $description === null])>
            @if ($title)
                <h2
                    @class(['text-xl tracking-tight text-pink-600 font-medium cursor-pointer', 'flex items-center gap-2' => $new])
            ...
            @if (! $disableCopy)
                <div class="flex rounded-md bg-transparent">
                    <button
                        ...
                        x-cloak
                    >
                        <x-icon name="code-bracket" solid class="dark:text-dark-400 h-5 w-5 text-gray-600" />
                    </button>
                </div>
            @endif
        </div>
    @endif
    @if ($description)
        <p class="dark:text-dark-400 mb-4 text-sm text-slate-500">{{ $description }}</p>
    @endif
