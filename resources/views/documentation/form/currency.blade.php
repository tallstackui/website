...
<x-layout :$content ai="form/currency">
    <x-slot:title>Form Currency</x-slot>
    <x-slot:description>Form currency component.</x-slot>
    ...
    <x-section title="Label & Hint">
        <x-preview language="blade" :contents="$labelHint">
            <x-currency
                ...
                hint="Between 5,000 and 10,000 USD"
            />
        </x-preview>
    </x-section>
    ...
    <x-section title="Different Currencies">
        <div class="space-y-4">
            <p>
                The
                <x-block>locale</x-block>
                attribute follows
                <x-block>Intl.NumberFormat</x-block>
                rules and defaults to en-US. Set it to any supported locale to
    ...
    <x-section title="Intl Options" disable-copy>
        <div class="space-y-4">
            <p>
                The component was built with two attributes that are intended to
                manipulate the JavaScript
                <x-block>Intl.NumberFormat</x-block>
                , called
                <x-block>decimals</x-block>
                and
                <x-block>precision</x-block>
                . You can use them to manipulate the JavaScript
                <x-block>Intl.NumberFormat</x-block>
                :
            </p>
    ...
    <x-section title="Symbols">
        <div class="space-y-4">
            ...
            <p>
                The symbols are not based on the
                <x-block>locale</x-block>
                attribute, but rather on your app's
                <x-block>app.locale</x-block>
                configuration. This is because you may want to display a
                currency for currencies other than your app's
                <x-block>app.locale</x-block>
                . If you want to replace the symbols at runtime, you can set a
                string value on it:
            </p>
    ...
    <x-section title="Mutate Mode" disable-copy>
        <div class="space-y-4">
            <p>
                With the
                <x-block>mutate</x-block>
                attribute, the component sends the formatted string
                <strong>exactly as it appears in the input</strong>
                , group separator and decimal separator included. Use it when
                you want to persist the user-facing representation verbatim
    ...
    <x-section title="Decimal Mode" disable-copy>
        <div class="space-y-4">
            <p>
                ...
                attribute strips the locale's group separator and normalizes the
                decimal separator to
                <x-block>.</x-block>
                , regardless of locale. The resulting string is directly
                castable via
                <x-block>(float)</x-block>
                /
                <x-block>(int)</x-block>
                or by Eloquent
                <x-block>decimal:N</x-block>
                /
                <x-block>float</x-block>
                casts.
            </p>
            ...
            />
            <p>
                A typical use case is binding to a column with an Eloquent
                <x-block>decimal:2</x-block>
                cast:
            </p>
    ...
    <x-section title="Sync Modes" disable-copy>
        <div class="space-y-4">
            <p>
                When bound to a Livewire property, the currency component offers
                three modes for sending the value to the server. Pick the one
                that matches how the value is persisted on the backend.
            </p>
            <x-table
                :headers="[
