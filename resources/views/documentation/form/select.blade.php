...
<x-layout :$content :ai="['Select Native' => 'form/select/native', 'Select Styled' => 'form/select/styled']">
    ...
    <x-slot:description>Select components.</x-slot:description>
    <x-slot:customization>
        <livewire:customization
            :customization="$customizationNative"
            component="Form\Select\Native"
            title="Select Native"
        />
        <livewire:customization
            :customization="$customizationStyled"
            component="Form\Select\Styled"
            title="Select Styled"
        />
    </x-slot:customization>
    ...
    >
        <div class="space-y-2">
            ...
            <p>
                When the value and label do not come from the <x-block>label</x-block> and
                <x-block>value</x-block> positions respectively, you must specify this manually using the
                <x-block>select</x-block> attribute:
            </p>
            <x-code language="blade" :contents="$nativeAdvancedDifferentIndexes" disable-copy />
        </div>
    </x-section>
    <x-warning> This option is also available for the other select components below. </x-warning>
    ...
    >
        <div class="space-y-4">
            ...
            <p>
                While there are no explicit examples in the documentation, you're free to use multiple
                <x-block>select.styled</x-block> on the same page, and also make options reactive based on other
                properties using <x-block>wire:model.live</x-block>. If you encounter any misbehavior in this regard,
                try to fully explore options like <x-block>wire:ignore</x-block> or <x-block>wire:replace</x-block>,
                with <x-block>.self</x-block> variants of the directives.
            </p>
        </div>
    </x-section>
    <x-section title="Label & Hint" anchor="styled-label-hint">
        <x-preview language="blade" :contents="$styledLabelHint">
            <x-select.styled label="Select One Option" hint="You can choose 1, 2 or 3" :options="[1, 2, 3]" />
        </x-preview>
    </x-section>
    ...
    <x-section
        ...
        description="An option to disable specific options."
        anchor="styled-disable-options"
    >
    ...
    <x-section title="Grouped Options" anchor="styled-grouped-options">
        <div class="space-y-4">
            <p>
                Options can be organized into groups. When an option's <x-block>value</x-block> is an array of
                sub-options, the component automatically detects grouped mode. Group headers display the label, optional
                description, and optional image, but only the nested items within groups are selectable.
            </p>
            ...
            <p>
                Search filters items within groups and hides groups with no matching items. Grouped options work with
    ...
    <x-section
        ...
        description="An option to limit the number of selections."
        anchor="styled-limit-selection"
    >
    ...
    <x-section title="Image Preview" anchor="styled-image-preview">
        <div class="space-y-4">
            <p>
                The styled select allows you to display an image next to the option label. To do this, the options
                simply have an index called <x-block>image</x-block> with the image URL.
            </p>
            ...
            <p>
                You can also set a different index to the <x-block>image</x-block> via the
                <x-block>select</x-block> attribute.
            </p>
    ...
    <x-section title="Description" anchor="styled-description">
        <div class="space-y-4">
            <p>
                You can also set a description for each option. To do this, the options simply have an index called
            ...
            <p>
                You can also set a different index to the <x-block>description</x-block> via the
                <x-block>select</x-block> attribute:
            </p>
    ...
    <x-section title="Lazy Load" anchor="styled-lazy-load">
        <div class="space-y-4">
            <p>
                Lazy loading is a way to slowly load options as you scroll down the floating element that displays the
                available options.
                <u>While this is extremely useful for delaying the loading of large amounts of options</u>, there is two
                major caveat:
            </p>
            <ul class="list-inside list-decimal">
                <li>
                    The <x-block>searchable</x-block> attribute will not work to search for options that have not yet
                    been loaded.
                </li>
    ...
    <x-section
        ...
        description="An option to interact with the search term when nothing is found."
    >
                ...
                <x-slot:after>
                    <div class="mb-2 flex items-center justify-center px-2">
                        <x-button
                            x-on:click="
                                show = false;
                                $tsui.interaction('dialog').success('Done!', `Term: ${search}`).send();
                            "
                        >
    ...
    <x-section
        ...
        description="Helpers to open and close the styled select using AlpineJS."
        anchor="styled-alpinejs-helper"
        disable-copy
    >
    ...
    <x-section title="Concept" anchor="styled-api-concept" disable-copy>
        The styled select component allows you to query for values via an API. In this mode, all the options available
        above remain available for use, the difference is that instead of setting the options using the
        <x-block>options</x-block> parameter, you must specify the API URL where the results will come from. For
        identification purposes, a header called <x-block>X-Tallstack-Ui</x-block> will be added to all requests.
    </x-section>
    <x-section title="Basic Usage" anchor="styled-api-basic-usage">
        <div class="space-y-4">
            ...
            <p>
                When the value and label do not come from the <x-block>label</x-block> and
                <x-block>value</x-block> positions respectively, you must specify this manually using the
                <x-block>select</x-block> attribute, like mentioned in the multi-dimensional array section. This example
                do not set the <x-block>select</x-block> attribute because the API returns the values in the correct
                format.
            </p>
        </div>
    </x-section>
    <x-section title="Unfiltered Results" anchor="styled-api-unfiltered-results">
        <div class="space-y-4">
            <p>
                The styled select component filters the API results using internal logic to search for the desired
                option based on the term entered in the input only in the item's
                <x-block>label</x-block> or <x-block>description</x-block>. You can disable this behavior to make the
                filters apply only in the API. In other words, you need to ensure that, for example, the value search
                applies a <x-block>where</x-block> based on the term sent in the search and appended to the query string
                <x-block>search</x-block>.
            </p>
            <x-preview language="blade" :contents="$styledApiUnfiltered">
                <x-select.styled :request="route('api.users')" unfiltered />
            </x-preview>
            <x-warning> This is only available in the styled API select. </x-warning>
            <p>In this example, the query was builded as follows:</p>
            <x-code :contents="$styledApiUnfilteredQuery" />
            <p>Which means:</p>
            <ul class="list-inside list-decimal">
                <li>
                    When <x-block>search</x-block> is set, then the query will be filtered by the
                    <x-block>name</x-block> column.
                </li>
                <li>When <x-block>search</x-block> is not set, then the query will return only 10 results.</li>
            </ul>
            <p>
                Optionally, you can set the <x-block>unfiltered</x-block> globally by the TallStackUI
    ...
    <x-section title="Advanced Usage" anchor="styled-api-advanced-usage">
        <div class="space-y-4">
            <p class="mt-4">
                You have the possibility to customize some things related to the request, such as the method used:
                <x-block>get</x-block> or <x-block>post</x-block>, and also add more <x-block>params</x-block> to the
                request. One of the big advantages of using these advanced options is the
                <x-block>params</x-block> parameter, because it is hydrated. This means that if you are using this
                component within the Livewire components and create a variable to be used in params , when making any
                changes to this variable and Livewire hydrates the page, the next time the select is opened to make a
                new request, params will be updated in the request.
            </p>
    ...
    <x-section title="Recycle" anchor="styled-api-recycle" disable-copy>
        <p>
            ...
            also control this globally in the
            <a href="{{ route('documentation', ['configuration']) }}" wire:navigate class="underline"
                >configuration file</a
