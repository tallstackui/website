...
<x-layout :$content ai="form/upload/async">
    ...
    <x-warning class="mb-4">
        The previews on this page post to a throwaway endpoint that accepts
        images up to 50 MB and discards them on a schedule. Nothing you drop
        here is kept.
    </x-warning>
    <x-section title="Concept" new disable-copy>
        <div class="space-y-4">
            <p>
                ...
                slices each file in the browser and posts the pieces straight to
                a controller of your own, bypassing the Livewire upload
                pipeline, so a file no longer has to fit inside the PHP request
                limits. Files around 1 GB are the target.
            </p>
            <p>
    ...
    >
        ...
        <x-warning warning title="directory is required" class="mt-4">
            ...
            callback, the handler throws instead of guessing a destination. A
            package-wide fallback would quietly pile every upload into one
            folder.
        </x-warning>
    </x-section>
    ...
    <x-section title="Guards" new disable-copy>
        <div class="space-y-4">
            <p>
                <x-block>max-size</x-block>
                on the component is feedback for the user. The handler re-checks
                the declared size on every chunk and compares the assembled
                bytes at the end, and
                <x-block>rules</x-block>
                run against the real bytes rather than the mime the browser
                claimed. Route middleware stays yours;
                <x-block>authorize</x-block>
                sits on top of it, running on every chunk before anything is
                written.
            </p>
    ...
    <x-section title="Staging" new disable-copy>
        <div class="space-y-4">
            <p>
                Chunks are staged as one part file per index and joined at the
                end. They are uploaded
                <x-block>concurrency</x-block>
                at a time and arrive out of order, so appending them to a single
                file would interleave the payload. One part per chunk removes
                ordering from the equation and turns
                <i>is it complete?</i>
                into a file count.
            </p>
            <p>
                Two atomic filesystem operations carry the coordination:
                creating the directory elects the request that fires
                <x-block>AsyncUploadStarted</x-block>
                , and renaming the staging directory elects the single request
                that finalizes. Counting parts alone is not enough, since two
                requests can observe a complete set at the same moment.
            </p>
            <x-warning class="mt-4">
                ...
                do not have. The finished file then goes wherever you name, S3
                included.
            </x-warning>
        </div>
    </x-section>
    ...
    >
        ...
        <x-warning
            warning
            title="AsyncUploadCompleted is not the place to write a database row"
            class="mt-4"
        >
            A finished upload is not a submitted form, and the user may still
            close the tab. That write belongs where the form is handled, reading
            the array the component synced out.
        </x-warning>
    </x-section>
    <x-section
        title="Configuration"
