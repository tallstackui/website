...
<x-layout :$content ai="form/upload/async">
    ...
    <x-section title="Concept" new disable-copy>
        <div class="space-y-4">
            ...
            <p>
                It shares nothing with <a href="{{ route('documentation', ['form', 'upload']) }}" wire:navigate class="underline">
                <x-block>&lt;x-upload&gt;</x-block></a> beyond the namespace, and unlike it, this one works outside
                Livewire too.
            </p>
        </div>
    </x-section>
    <x-section title="Basic Usage" new description="route accepts a named route or a plain URL. Either wire:model or name is required.">
    ...
    <x-section title="Manual" new description="Stages the files and waits for the Send button. The footer slot replaces the built-in Send and Clear buttons.">
        <x-preview language="blade" :contents="$manual">
            <livewire:documentation.form.upload.async :mode="3" />
        </x-preview>
    </x-section>
    <x-section title="Without Livewire" new description="The same array is rendered as hidden inputs, so a plain form submit carries it.">
        <x-code language="blade" :contents="$native" />
    </x-section>
    <x-section title="Bound Value" new description="With multiple off, a single object or null. url is null when the destination disk has no public URL.">
        <x-code language="php" :contents="$value" />
    </x-section>
    <x-section title="Backend" new description="The method is called once per chunk. Intermediate chunks answer 204; the last one assembles the file, validates it, stores it and answers 200.">
    ...
    <x-section title="Taking Over Persistence" new description="Runs once per file, after the pieces are joined and validated. The returned path must exist on the disk named in disk.">
    ...
    <x-section title="Staging" new disable-copy>
        <div class="space-y-4">
            ...
            <p>
                ...
                fires <x-block>AsyncUploadStarted</x-block>, and renaming the staging directory elects the single request
                that finalizes. Counting parts alone is not enough, since two requests can observe a complete set at the
                same moment.
            </p>
    ...
    <x-section title="Cleaning Up" new description="An upload that starts and never finishes leaves its pieces staged. Nothing else collects them. Finalized files are never touched.">
    ...
    <x-section title="Laravel Events" new description="For side effects: queueing a thumbnail, scanning, auditing. There is deliberately no per-chunk event.">
        <x-table :headers="[
            ['index' => 'event', 'label' => 'Event'],
            ['index' => 'when', 'label' => 'When'],
    ...
    <x-section title="Configuration" new description="Under components.upload.async. Raising chunk_size requires raising the PHP limits with it, or every chunk is rejected before reaching Laravel.">
        <x-table :headers="[
