...
<x-layout :$content ai="form/color">
    <x-slot:title>Form Color</x-slot>
    <x-slot:description>Form color component.</x-slot>
    ...
    <x-section
        title="Selectable"
        description="An option to force the color selection through the color selector box."
    >
        <x-preview language="blade" :contents="$selectable">
            <x-color selectable picker />
        </x-preview>
        <x-warning class="mt-4">
            This will transform the input in readonly and open the color
            selector box when the input is clicked.
        </x-warning>
    </x-section>
    <x-section
        title="Custom Colors"
        description="An option to specific your own hexadecimal colors."
    >
        <x-preview language="blade" :contents="$custom">
            <x-color
                :colors="['#83493D', '#3D8357', '#693D83', '#3AB3D1', '#5DD116']"
            />
        </x-preview>
    </x-section>
    ...
    <x-section
        title="Exclude Step"
        description="An option to exclude a step from the color selector in picker mode."
    >
        <x-preview language="blade" :contents="$excludeStep">
            <x-color picker excluded-step="500" />
        </x-preview>
        <x-warning class="mt-4">
            Excluded step can only be used with
            <x-block>picker</x-block>
            mode.
        </x-warning>
    </x-section>
    <x-section
