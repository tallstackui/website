...
<x-layout :$content>
    ...
    <x-section title="Concept" disable-copy>
        <div class="space-y-4">
            ...
            <div class="mt-2 space-y-2">
                <p class="text-md font-medium">Components that CANNOT be used out of a Livewire component ❌</p>
                ...
                <p>
                    Any other component not listed above can be used outside of Livewire components. Livewire's script
                    still has to be on the page, since that is where AlpineJS comes from.
                </p>
            </div>
            <x-warning warning title="Table left the list on 4.x">
                <x-block>x-table</x-block> no longer requires the Livewire context, and
                <x-block>x-upload.async</x-block> works outside it too &mdash; only the original
                <x-block>x-upload</x-block> still rides the Livewire upload pipeline.
            </x-warning>
        </div>
    </x-section>
    ...
    <x-section title="Table" new disable-copy>
        <div class="space-y-4">
            <p>
                Pagination becomes anchors built from the URLs the paginator already exposes, sorting becomes an anchor
                carrying the inverted direction, and the filter rewrites <x-block>location</x-block> through AlpineJS.
                <x-block>loading</x-block> needs <x-block>wire:loading</x-block> and is not rendered.
            </p>
