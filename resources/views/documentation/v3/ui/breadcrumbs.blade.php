...
<x-layout :$content>
    ...
    <x-section title="Concept" disable-copy>
        <div class="space-y-4">
            <p>
                The breadcrumbs component displays a navigation trail of links helping users
                understand their position within the application hierarchy. You can provide items
                manually as an array or let the component auto-resolve them from a route-based
                breadcrumb registry.
            </p>
        </div>
    </x-section>
    ...
    <x-section title="Named Routes">
        <div class="space-y-4">
            <p>
                The <x-block>link</x-block> field accepts both URLs and Laravel named routes.
                Values starting with <x-block>/</x-block> or <x-block>http</x-block> are kept as-is.
    ...
    <x-section title="Route-Aware Builder" disable-copy>
        <div class="space-y-4">
            <p>
                Instead of passing items manually, you can register breadcrumb definitions
                for named routes and let the component auto-resolve the trail based on the
                current route. There are two ways to register definitions:
            </p>
            <p><b>1)</b> Publish a dedicated breadcrumb file (recommended):</p>
            <x-code language="shell" :contents="$publishCommand" />
            <p>
                This creates a <x-block>routes/breadcrumbs.php</x-block> file. Define your breadcrumbs there:
            </p>
            <x-code language="php" :contents="$registryFile" />
            <p>
                You can configure additional breadcrumb files in <x-block>config/tallstackui.php</x-block>.
                Paths are relative to <x-block>base_path()</x-block> and non-existent files are
                silently skipped:
            </p>
            <x-code language="php" :contents="$configFiles" />
            <p><b>2)</b> Register breadcrumbs in any service provider's <x-block>boot</x-block> method:</p>
            <x-code language="php" :contents="$serviceProvider" />
            <p>
                Once registered, use the component without passing <x-block>items</x-block> to
                auto-resolve breadcrumbs from the current route:
            </p>
    ...
    <x-section title="Route Model Binding" disable-copy>
        <div class="space-y-4">
            <p>
                Callback parameters are automatically injected from the current route's model
                bindings via Laravel's container. Type-hint any route-bound model to receive it:
            </p>
