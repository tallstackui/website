<x-layout :$content :torchlight="false">
    ...
    <x-section disable-copy>
        <p>
            TallStackUI offers two customization approaches. One that we call <x-block>soft</x-block> because it is easy and simple,
    ...
    <x-section class="grid grid-cols-2 gap-4" disable-copy>
        <x-card>
            <a class="inline-flex items-center gap-2" href="{{ route('documentation', ['v3', 'customization', 'soft']) }}" wire:navigate>
                <u>Soft</u> Customization
                <x-icon name="arrow-up-right" class="h-5 w-5 text-pink-500" />
            </a>
        </x-card>
        <x-card>
            <a class="inline-flex items-center gap-2" href="{{ route('documentation', ['v3', 'customization', 'deep']) }}" wire:navigate>
