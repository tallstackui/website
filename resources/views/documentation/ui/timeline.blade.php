@php
    ...
    $perItemColor = [["title" => "Default", "date" => "Q1"], ["title" => "Green override", "date" => "Q2", "color" => "green"], ["title" => "Red override", "date" => "Q3", "color" => "red"]];
...
<x-layout
    :$content
    :ai="['Timeline' => 'timeline/main', 'Timeline Items' => 'timeline/items']"
>
    <x-slot:title>Timeline</x-slot>
    <x-slot:description>Timeline component.</x-slot>
    <x-slot:customization>
        <livewire:customization
            :$customization
            component="Timeline\Main"
            title="Timeline"
        />
        <livewire:customization
    ...
    <x-section title="Render Modes">
        <div class="space-y-4">
            <p>
                Timeline supports two render modes: passing items via the
                <x-block>:items</x-block>
                prop (array or Collection)
                <b>or</b>
                declaring children as
                <x-block>&lt;x-timeline.items&gt;</x-block>
                in a slot. Combining both raises
    ...
    <x-section title="Alternate Layout">
        ...
        <x-warning class="mt-4">
            Alternate uses a 3-column grid that distributes the timeline's
            horizontal space. Wrap it in a max-w-* container or apply
            class="max-w-*" to the timeline so the line lands at a comfortable
            midpoint instead of stretching across the full parent width.
        </x-warning>
    </x-section>
    ...
    <x-section title="Custom Marker Slot">
        <x-preview language="blade" :contents="$markerSlot">
            <x-timeline color="rose">
                <x-timeline.items
                    title="Numbered step 1"
                    description="First step."
                >
                    <x-slot:marker>
                        <span
                            class="flex h-6 w-6 items-center justify-center rounded-full bg-rose-600 text-xs font-bold text-white"
                        >
                            1
                        </span>
                    </x-slot>
