...
<x-layout :$content ai="card">
    ...
    <x-slot:description>Card component.</x-slot:description>
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <x-preview language="blade" :contents="$minimizeFromMount">
                    <x-card header="TallStackUI" minimize initialize-minimized> Initialized as minimized </x-card>
                </x-preview>
            </div>
        </div>
    </x-section>
    <x-section title="Image">
        <x-preview language="blade" :contents="$image">
            <div class="space-y-4">
                <x-card image="{{ url('assets/images/wallpapers/1.jpg') }}"> Image at top </x-card>
                <x-card position="bottom" image="{{ url('assets/images/wallpapers/2.jpg') }}"> Image at bottom </x-card>
            </div>
        </x-preview>
    </x-section>
    <x-section title="Round" description="An option to control the border radius of the card wrapper.">
        ...
        <x-warning class="mt-4">
            When omitted or used as a bare flag, <x-block>round</x-block> keeps the historical default
            (<x-block>rounded-lg</x-block>). Pass a size between <x-block>xs</x-block> and <x-block>2xl</x-block> to
            override the wrapper radius.
        </x-warning>
    </x-section>
    <x-section title="Color Variations">
        <x-preview language="blade" :contents="$color">
            <div class="space-y-4">
                ...
                <x-card header="TallStackUI" color="primary" light> Normal, Light </x-card>
                <x-card header="TallStackUI" color="primary" accent> Accent </x-card>
            </div>
        </x-preview>
        <x-warning class="mt-4">
        ...
        <x-warning warning title="bordered became accent" class="mt-4">
            On <x-block>3.x</x-block> this variation was called <x-block>bordered</x-block>. That name now belongs to
            the wrapper border below, so combining <x-block>color</x-block> with a colored top border is
    ...
    <x-section title="Paddingless" new>
        <x-preview language="blade" :contents="$paddingless">
            <x-card header="Card without padding in the body" paddingless> TallStackUI </x-card>
        </x-preview>
    </x-section>
    ...
    <x-section title="Skeleton" new description="An option to display loading skeleton in the card.">
        ...
        <p class="mt-4">
            Example of usage using <x-block>placeholder()</x-block> of a <x-block>#[Lazy]</x-block> component:
        </p>
    ...
    <x-section title="Loading" description="An option to set the card in a loading state.">
        ...
        <p class="mt-4">
            Even if you use the <x-block>header</x-block> slot to set a raw HTML content the loading effect will be
            visible
        </p>
    </x-section>
