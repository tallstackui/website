...
<x-layout :$content ai="qr-code">
    ...
    <x-section title="Concept" disable-copy>
        A dependency-free QR code rendered as inline SVG. The whole of ISO/IEC 18004 lives in the package: nothing is
        fetched and no encoding library is involved. AlpineJS is attached only when the code offers copy or download.
    </x-section>
    ...
    <x-section title="Colors" new description="Without color the modules follow the theme, which inverts the symbol in dark mode. Pass an explicit color where reader conformance matters.">
    ...
    <x-section title="Size Variations" new description="size sets only the rendered box. The module count comes from the payload.">
    ...
    <x-section title="Watermark" new description="The modules underneath are removed rather than covered, and the error correction level rises to H on its own.">
    ...
    <x-section title="Copy & Download" new description="Copy always writes a PNG. Download takes png or svg. The export carries no background, so the PNG is transparent.">
    ...
    <x-section title="Skeleton" new description="Renders a placeholder and stops requiring a link, which is what a placeholder stands in for.">
        <x-preview language="blade" :contents="$skeleton">
            <x-qr-code skeleton size="lg" />
        </x-preview>
    </x-section>
    <x-section title="Configuration" new description="size is only consulted when the attribute is absent. pixels is the width of the exported image and has no effect on the page.">
        <x-code language="php" :contents="$configuration" />
    </x-section>
    <x-section title="Scannability" new description="Pixels per module at one times. Below roughly three the code stops being readable from a 1x display. A retina display doubles every number.">
        <x-table :headers="[
