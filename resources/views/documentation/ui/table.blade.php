...
<x-layout :$content ai="table">
    ...
    <x-slot:description>Table component.</x-slot:description>
    ...
    <x-section title="Data for Table" disable-copy>
        <p>
            You have two ways to provide data to create a table: <b>1)</b> Simple data through an array or
            <b>2)</b> Data that comes from the database, using
            <a
                href="https://laravel.com/docs/10.x/pagination#paginating-eloquent-results"
                class="underline"
                target="_blank"
            >
                Laravel Eloquent Pagination.</a>
            The main difference between the two ways is that when choosing to create a
            <u
                >table with an array of data, features such as filtering, sorting and pagination will be more difficult
                to implement.</u>
        </p>
    ...
    <x-section title="Interacting With Data" disable-copy>
        ...
        <p>
            <a
                ...
                target="_blank"
            >
                As mentioned in the Livewire documentation</a
            >, for cases where you want to render components in a loop, using the Blade
            <x-block>@verbatim @interact @endverbatim</x-block>
            directive, you must specify a unique key for each component:
        </p>
        <x-code :contents="$loop" disable-copy />
        <p>
            You can use <x-block>$loop</x-block> inside the
            <x-block>@verbatim @interact @endverbatim</x-block> directive to interact with the data of each row. The
            <x-block>$loop</x-block> variable is a special variable provided by Blade that contains information about
    ...
    >
        ...
        <x-warning class="mt-4">
            ...
            paginator is also passed through <x-block>withQueryString()</x-block>, or paginating would silently drop the
    ...
    >
        <x-preview language="blade" :contents="$paginators" :background="false">
            <div class="space-y-8">
                @foreach (['simple', 'minimal', 'compact'] as $variant)
                <livewire:documentation.ui.table :mode="14" :paginator="$variant" :key="'paginator-'.$variant" />
                @endforeach
            </div>
        </x-preview>
        <x-table
    ...
    >
        <x-preview language="blade" :contents="$simplePagination" :background="false">
            <div class="space-y-8">
                @foreach (['simple', 'minimal', 'compact'] as $variant)
                <livewire:documentation.ui.table :mode="15" :paginator="$variant" :key="'simple-paginator-'.$variant" />
                @endforeach
            </div>
        </x-preview>
    </x-section>
    ...
    >
        <x-preview language="blade" :contents="$persistent" :background="false">
            <div id="users-table">
    ...
    >
        ...
        <p class="mt-4">
            It belongs in the <x-block>placeholder()</x-block> of a <x-block>#[Lazy]</x-block> component:
        </p>
