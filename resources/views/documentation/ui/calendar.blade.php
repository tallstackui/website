...
<x-layout :$content ai="calendar">
    ...
    <x-slot:description>Calendar component (inline date picker).</x-slot:description>
    <x-slot:customization>
        <livewire:customization :$customization component="Calendar" title="Calendar" />
    </x-slot:customization>
    <x-section title="Basic Usage">
        <div class="space-y-4">
            <p>
                Calendar is an <b>inline</b> date picker, the same grid-based UX you get from
                <x-block>&lt;x-date&gt;</x-block>, but rendered directly in the page with no input field and no floating
                wrapper. Useful for dashboards, filter panels, scheduling screens, and inline reports.
            </p>
    ...
    <x-section title="Label & Hint">
        <x-preview language="blade" :contents="$labelHint" :background="false">
    ...
    <x-section title="Range Mode">
        <x-preview language="blade" :contents="$range" :background="false">
            <x-calendar range :value="[now()->addDays(2)->format('Y-m-d'), now()->addWeek()->format('Y-m-d')]" />
        </x-preview>
    ...
    >
        <x-preview language="blade" :contents="$double" :background="false">
            <x-calendar range double :value="[now()->format('Y-m-d'), now()->addDays(20)->format('Y-m-d')]" />
        </x-preview>
        <x-warning class="mt-4">
    ...
    >
        <x-preview language="blade" :contents="$lockMonthYear" :background="false">
            <x-calendar lock-month-year />
        </x-preview>
        <x-warning class="mt-4">
            lock-month-year and month-year-only cannot be used together, month-year-only would freeze the component
            because the picker is the only interaction surface.
        </x-warning>
    </x-section>
    ...
    <x-section title="Other Options" disable-copy>
        <p>
            In addition to the methods demonstrated above, all other methods available in the
            <a href="{{ route('documentation', ['form', 'date']) }}" wire:navigate class="underline">date</a> component
            are also available in the Calendar component.
        </p>
    </x-section>
</x-layout>
