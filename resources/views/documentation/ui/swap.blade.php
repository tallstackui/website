...
<x-layout :$content ai="swap">
    ...
    <x-section title="Concept" new disable-copy>
        A compact value cycler shaped like an input: a chevron button on each side, the selected value in the middle.
        The value moves through the buttons, through a drag over the value itself — pointer events, so mouse and touch
        behave identically — or through the keyboard arrows while either button holds focus. The middle is deliberately
        not focusable: <x-block>Tab</x-block> stops only on the buttons.
    </x-section>
    ...
    <x-section title="Vertical" new description="Rolls the value top-to-bottom instead. The chevrons become up and down, and the drag axis follows.">
        ...
        <x-warning class="mt-4">
            <x-block>preview</x-block> and <x-block>vertical</x-block> cannot be combined, because sideways slices make
            no sense on a vertical roll. The render throws.
        </x-warning>
    </x-section>
    <x-section title="Block" new description="Stretches the control to the full width of the parent.">
    ...
    <x-section title="Loop" new description="Navigation loops by default: crossing an edge animates into a clone of the opposite end and silently teleports to the real option, so the cycle reads as continuous.">
        ...
        <x-warning class="mt-4">
            With <x-block>:loop="false"</x-block> the matching button disables at either end and the drag gains rubber
    ...
    <x-section title="Motion" new disable-copy>
        The track slides on <x-block>transform</x-block> inside an overflow viewport, 300ms ease-out by default,
        degrading to an instant jump under <x-block>globals()->flash()</x-block>. During a drag the transition is
        suspended so the value follows the pointer 1:1 — a long gesture crosses several options — and the release snaps
        to the nearest one.
    </x-section>
