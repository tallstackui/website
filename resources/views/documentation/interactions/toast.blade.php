...
<x-layout :$content ai="toast">
    ...
    <x-slot:description>Toast component.</x-slot:description>
    ...
    <x-section title="Expandable" description="An option to show minimized description texts.">
        <div class="space-y-4">
            ...
            <p class="mt-2">
                ...
                <a href="{{ route('documentation', ['configuration']) }}" wire:navigate class="underline"
                    >the configuration file.</a>
                When you do that, you can optionally ignore the expandable for specific Toast:
            </p>
    ...
    <x-section title="Position" description="An option to control the toast position at runtime.">
        <div class="space-y-4">
            ...
            <p class="mt-2">
                If you quickly click on all the buttons above, you will see that the position of the Toasts that were
                already displayed will change. This happens because the
    ...
    <x-section title="Sole">
        <div class="space-y-4">
            <p class="mt-2">
                Starting from version v2.9.0, you can use the <x-block>sole</x-block> method to clear all other
                displayed Toasts in favor of displaying only a specific Toast.
            </p>
    ...
    <x-section title="Hooks">
        <div class="space-y-4">
            ...
            <p>
                <u>The closure will be resolved using Laravel container</u> and the result will be passed to the hook.
            </p>
        </div>
    </x-section>
    <x-section title="JavaScript API" description="JavaScript API to interact with Toast.">
        <div class="space-y-4">
            ...
            <x-warning> The toast hooks are unavailable in the JavaScript API. </x-warning>
        </div>
    </x-section>
    <x-section title="Display After Redirects">
        <div class="space-y-4">
            <p>
                ...
                the redirection is to another Livewire component or to basic controllers, you can still display the
                toast.
            </p>
            <x-code :contents="$flash" />
            <p>
                When using <x-block>flash()</x-block> method, the toast will not be displayed in the current request,
                but yes stored temporarily in the session and displayed in the next request.
            </p>
        </div>
    </x-section>
    <x-section title="Dispatching via Controllers">
        <div class="space-y-4">
            ...
            <p>
                <u>Because certain methods were created and designed to be used with Livewire components</u>, methods
                like <x-block>confirm</x-block>, <x-block>cancel</x-block> and <x-block>hooks</x-block>
    ...
    <x-section title="Colorful" disable-copy>
        <p>
            You can invert the Toast colors using the <x-block>colorful</x-block> global so the body background takes
            the notification type color (green for success, red for error, and so on) with white text.
