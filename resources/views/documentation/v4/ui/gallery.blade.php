...
<x-layout :$content ai="gallery">
    ...
    <x-section title="Concept" disable-copy>
        An image gallery with three layouts and an opt-in lightbox. The layout is chosen by a boolean flag, and the
        flags are mutually exclusive. Without <x-block>clickable</x-block> the component emits no JavaScript at all.
    </x-section>
    ...
    <x-section title="Grid" new description="Uniform tiles in a responsive grid. Tile shape comes from ratio: square, video or portrait.">
        <x-preview language="blade" :contents="$grid">
            <x-gallery grid :columns="4" ratio="portrait" :images="$images" />
        </x-preview>
    </x-section>
    <x-section title="Masonry" new description="Equal-width columns where each image keeps its natural height. Reading order runs down each column. ratio does not apply.">
        <x-preview language="blade" :contents="$masonry">
            <x-gallery masonry :columns="3" :images="$images" />
        </x-preview>
    </x-section>
    <x-section title="Feature" new description="One cover above a thumbnail row. When the array holds more images than limit, the last thumbnail gets a +N overlay.">
        <x-preview language="blade" :contents="$feature">
            <x-gallery feature :limit="3" :images="$images" />
        </x-preview>
    </x-section>
    <x-section title="Thumbnail Position" new description="Moves the row beside the cover. Below sm the thumbnails always wrap under it.">
    ...
    <x-section title="Height" new description="Caps the tile area and scrolls inside it: 40, 60, 80 or 96. The slots stay outside the scrolling region.">
        <x-preview language="blade" :contents="$height">
            <x-gallery grid height="60" :columns="3" :images="$images" />
        </x-preview>
    </x-section>
    <x-section title="Clickable" new description="Expands a tile fullscreen through an overlay teleported to the body. Closes on Esc, on the backdrop and on the close button.">
        <x-preview language="blade" :contents="$clickable">
            <x-gallery clickable :images="$images" />
        </x-preview>
    </x-section>
    <x-section title="Caption" new description="overlay keeps the caption on the image, footer separates it onto its own row.">
    ...
    <x-section title="Navigable" new description="Side arrows and the arrow keys inside the lightbox. without-loop stops the traversal at both ends.">
    ...
    <x-section title="Events" new description="Dispatched on the root element when clickable is enabled.">
    ...
    <x-section title="Performance" new description="Every tile carries loading=&quot;lazy&quot;, decoding=&quot;async&quot; and content-visibility: auto. The last one lives in its own block and can be dropped.">
        <x-code language="php" :contents="$performance" />
    </x-section>
    <x-section title="Validation" new description="Attributes passed to a layout that ignores them raise instead of being dropped.">
        <x-table :headers="[
