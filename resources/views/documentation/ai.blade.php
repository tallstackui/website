...
<x-layout :$content>
    <x-slot:title>AI</x-slot:title>
    <x-slot:description>AI-powered development with TallStackUI.</x-slot:description>
    <x-section title="AI-Powered Development" disable-copy>
        <div class="space-y-4">
            <p>
                ...
                full component knowledge. Each component has a dedicated markdown file with attributes, slots, usage
                examples, and customization options. In addition, TallStackUI provides an <b>MCP server</b> hosted on
                this documentation website, giving AI assistants real-time access to component documentation without
                needing local files.
            </p>
        </div>
    </x-section>
    <x-section title="Component Instructions" disable-copy>
        <div class="space-y-4">
            <p>
                The following component instruction files are available in the <x-block>.ai/</x-block> directory of the
                TallStackUI package. Each file contains comprehensive documentation for AI assistants, including
            ...
            >
                @interact('column_file', $row)
                    <div class="flex items-center gap-2">
                        <a href="{{ $row['url'] }}" class="text-pink-600 underline dark:text-pink-400" target="_blank">
                            {{ $row['file'] }}
                        </a>
    ...
    <x-section title="MCP Server" disable-copy>
        <div class="space-y-4">
            <p>
                The
                <a href="https://modelcontextprotocol.io" target="_blank" class="underline"
    ...
    <x-section title="Available Tools" disable-copy>
        <div class="space-y-4">
            <p>The MCP server provides the following tools for AI assistants:</p>
            <x-table
                :headers="[
    ...
    <x-section title="Available Resources" disable-copy>
        <div class="space-y-4">
            <p>
                Beyond the tools, the MCP server also exposes read-only resources that AI assistants can load to gain
                broader context in a single read:
            </p>
            <x-table
                :headers="[
            ...
            >
                ...
                @interact('column_description', $row)
                    ...
                    @else
                        ...
                        declared when a component renders nested components. Required reading before customizing nested
    ...
    <x-section title="Available Prompts" disable-copy>
        <div class="space-y-4">
            <p>
                The MCP server also ships prompts: ready-made workflows that guide AI assistants step by step through
                common tasks:
            </p>
            <x-table
                :headers="[
            ...
            >
                ...
                @endinteract
                @interact('column_description', $row)
                    Guided workflow to customize the CSS of a component through Soft Customization: it fetches the
                    component blocks, locates the target classes, resolves nested scopes, and writes the
    ...
    <x-section title="Connecting to Claude Code">
        <div class="space-y-4">
            <p>
                ...
                <a href="https://claude.ai/code" target="_blank" class="underline">Claude Code</a>
            ...
            <p>
                ...
                file in your project root. This file can be committed to version control so all team members have
                access:
            </p>
    ...
    <x-section title="Connecting to Cursor">
        <div class="space-y-4">
            ...
            <p>
                You can also create this file at <x-block>~/.cursor/mcp.json</x-block> for global access across all
                projects.
            </p>
        </div>
    </x-section>
    <x-section title="Connecting to Gemini CLI">
        <div class="space-y-4">
            <p>
                The quickest way to connect the MCP server to
                <a href="https://github.com/google-gemini/gemini-cli" target="_blank" class="underline">Gemini CLI</a>
                is using the CLI command:
            </p>
            <x-code language="shell" :contents="$geminiCli" />
            <p>Alternatively, add the server to your <x-block>~/.gemini/settings.json</x-block>:</p>
