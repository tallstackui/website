...
<x-layout :$content>
    ...
    <x-section title="Global Keys" new description="Top-level keys applying across components rather than to a single one. Per-component options live under components.&lt;name&gt; and are documented on each component's page.">
        ...
        <x-warning class="mt-4">
            ...
            Password, Select Styled, Time, Upload, Calendar and the List Items menu &mdash; every component built on
            the <a href="{{ route('documentation', ['internal', 'floating']) }}" wire:navigate class="underline">floating</a>.
        </x-warning>
    </x-section>
    <x-section title="How the Published File Is Merged" new description="Keys the file does not mention keep their default, so a file written against an older release does not lose options added since.">
        <div class="space-y-4">
            <p>
                Lists of scalars are the exception: they are taken as published rather than merged entry by entry,
                which is what lets a published list be shorter than the default.
            </p>
            ...
            <x-warning warning title="This was broken before 4.x">
                ...
                tightened. The same now applies to <x-block>table.quantity</x-block>, <x-block>editor.toolbar</x-block>
                and <x-block>debug.environments</x-block>.
            </x-warning>
        </div>
    </x-section>
    <x-section title="Scrollbar Offset" new description="Every overlay that locks the page gives the scrollbar's width back as padding-right, so the content does not jump sideways. That width is measured now instead of being a hardcoded 15px.">
        <div class="space-y-4">
            <p>
