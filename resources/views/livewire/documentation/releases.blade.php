...
new class extends Component {
    ...
    public function updatedBreaking(): void
    {
        $this->resetPage();
    }
    ...
    public function rows(): LengthAwarePaginator
    {
        $releases = RetrieveReleases::execute()
            ->filter(fn (array $release) => $release['draft'] === false && $release['prerelease'] === false)
            ->when($this->breaking, fn (Collection $releases) => $releases->filter(fn (array $release) => str_contains($release['body'], 'Breaking Change')))
        ...
        return new LengthAwarePaginator($releases->forPage($this->getPage(), 10), $releases->count(), 10, $this->getPage());
    }
};
...
<div>
    ...
    <x-table :$headers :rows="$this->rows" paginate simple-pagination>
