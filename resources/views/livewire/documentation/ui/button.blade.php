...
<div>
    @if ($model === 1)
        ...
        @elseif ($type === 2)
            <x-button wire:click="target" loading="target"> Loading Target </x-button>
        @else
            <x-button wire:click="delay" loading delay="longest"> Delay Control </x-button>
        @endif
    @elseif ($model === 2)
        @if ($type === 1)
        ...
        @endif
    @elseif ($model === 3)
        <x-button wire:click="target" loading="target" :spinner="$spinner"> {{ $spinner ?? 'default' }} </x-button>
    @elseif ($model === 4)
