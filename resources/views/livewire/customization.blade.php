...
<div class="shrink-0">
    <x-modal scrollable id="{{ $id }}">
        ...
        @endif
        <div wire:loading wire:target="open, select" class="flex items-center justify-center py-4">
            <x-spinner color="pink" />
        </div>
        @if ($blocks)
            <div wire:loading.remove wire:target="open, select" class="px-2 sm:px-0">
