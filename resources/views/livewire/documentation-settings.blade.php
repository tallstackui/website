    ...
    public function save(): void
    {
        $this->withValidator(function (Validator $validator) {
            $validator->after(function ($validator) {
                if (
                    $this->prefix &&
                    ! preg_match('/^[A-Za-z-]+$/', $this->prefix)
                ) {
                    return $validator
                        ->errors()
                        ->add(
                            "prefix",
                            "The prefix must contain only letters and dashes.",
                        );
                }
            });
        })->validate();
        ...
        Cookie::queue(
            blank($this->prefix)
                ? Cookie::forget("prefix")
                : Cookie::forever("prefix", $this->prefix),
        );
...
<div class="ml-0.5">
    ...
    >
        <p class="mb-4 text-sm">
            You can use this form to configure all configurable aspects of the
            TallStackUI docs.
        </p>
        <form
            id="form-documentation-settings"
            class="space-y-4"
            wire:submit="save"
        >
            <x-input
        ...
        <x-slot:footer>
            <x-button
                block
                type="submit"
                form="form-documentation-settings"
                color="pink"
                round
                sm
            >
